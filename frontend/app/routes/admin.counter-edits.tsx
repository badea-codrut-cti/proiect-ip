...
export default function AdminCounterEdits() {
    ...
    const fetchEdits = async () => {
        try {
            ...
            const data = await apiFetch<PendingEdit[]>("/api/counter-edits/pending");
            setEdits(data);
        } catch (err) {
    ...
    const handleApprove = async (id: string) => {
        if (!confirm("Are you sure you want to approve this documentation update? This will overwrite the current documentation.")) return;
        setProcessingId(id);
        try {
            ...
            <main className="max-w-6xl mx-auto px-4 py-12">
                ...
                {loadingEdits ? (
                    <div className="grid gap-6">
                        {[1, 2].map(i => (
                            <div key={i} className="h-64 bg-white dark:bg-slate-900 animate-pulse rounded-2xl border border-slate-200 dark:border-slate-800" />
                        ))}
                    </div>
                ) : edits.length === 0 ? (
                    <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 border-dashed">
                        <div className="text-4xl mb-4">✨</div>
                        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">Clean Slate!</h2>
                        <p className="text-slate-500 dark:text-slate-400">There are no pending documentation edits to review.</p>
                    </div>
                ) : (
                    <div className="grid gap-8">
                        {edits.map((edit) => (
                            <Card key={edit.id} className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <CardDescription className="flex items-center gap-3">
                                                <span className="flex items-center gap-1"><User className="h-3 w-3" /> {edit.created_by_username}</span>
                                                <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {new Date(edit.created_at).toLocaleDateString()}</span>
                                            </CardDescription>
                                        </div>
                                    </div>
                                ...
                                <CardContent className="pt-6">
                                    <div className="grid lg:grid-cols-3 gap-8">
                                        <div className="lg:col-span-2 space-y-4">
                                            <div>
                                                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Proposed Content</h4>
                                        ...
                                        <div className="space-y-6 pt-6 lg:pt-0 lg:border-l border-slate-100 dark:border-slate-800 lg:pl-8">
                                            <div className="space-y-3">
                                                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Decision</h4>
                                                <Button
                                                    className="w-full bg-emerald-600 hover:bg-emerald-700 text-white border-none"
                                                    onClick={() => handleApprove(edit.id)}
                                                    disabled={processingId !== null}
                                                >
                                                    {processingId === edit.id ? "Processing..." : (
                                            ...
                                            <div className="space-y-3">
                                                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">Reject with Feedback</h4>
                                                <Textarea
                                                    ...
                                                    onChange={(e) => setRejectionReason({ ...rejectionReason, [edit.id]: e.target.value })}
                                                />
                                                <Button
                                                    variant="outline"
