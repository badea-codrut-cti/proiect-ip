...
import { BookOpen, Check, X, User, Clock, Hash, Percent } from "lucide-react";
...
export default function AdminExercises() {
    ...
    const fetchExercises = async () => {
        try {
            ...
            const data = await apiFetch<PendingExercise[]>("/api/exercises/pending");
            setExercises(data);
        } catch (err) {
    ...
    const handleApprove = async (id: string) => {
        if (!confirm("Are you sure you want to approve this exercise? It will become available to all users.")) return;
        setProcessingId(id);
        try {
            ...
            <main className="max-w-6xl mx-auto px-4 py-12">
                ...
                {loadingExercises ? (
                    <div className="grid gap-6">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="h-44 bg-white dark:bg-slate-900 animate-pulse rounded-2xl border border-slate-200 dark:border-slate-800" />
                        ))}
                    </div>
                ) : exercises.length === 0 ? (
                    <div className="p-12 text-center bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 border-dashed">
                        <div className="text-4xl mb-4">🎉</div>
                        <h2 className="text-lg font-semibold text-slate-900 dark:text-slate-100">All Done!</h2>
                        <p className="text-slate-500 dark:text-slate-400">There are no pending exercise proposals at the moment.</p>
                    </div>
                ) : (
                    <div className="grid gap-6">
                        {exercises.map((ex) => (
                            <Card key={ex.id} className="border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 overflow-hidden shadow-sm">
                                        ...
                                        <div className="lg:col-span-3 p-6 space-y-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className="text-sm font-semibold text-slate-900 dark:text-slate-100">{ex.counter_name}</span>
                                                    <span className="h-1 w-1 rounded-full bg-slate-300 dark:bg-slate-700" />
                                                    <span className="text-xs text-slate-500 dark:text-slate-400">Proposed by {ex.created_by_username}</span>
                                            ...
                                            <div className="flex flex-wrap gap-4 pl-11">
                                                <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 px-2 py-1 rounded-md border border-slate-100 dark:border-slate-800">
                                                    <span className="font-bold text-slate-700 dark:text-slate-300">Range:</span>
                                                    {ex.min_count} • {ex.max_count}
                                                </div>
                                                <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 px-2 py-1 rounded-md border border-slate-100 dark:border-slate-800">
                                                    <Percent className="h-3 w-3" />
                                                    <span className="font-bold text-slate-700 dark:text-slate-300">Decimals:</span>
                                                    {ex.decimal_points}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="p-6 bg-slate-50/50 dark:bg-slate-800/20 border-t lg:border-t-0 lg:border-l border-slate-100 dark:border-slate-800 space-y-4">
                                            ...
                                            <div className="space-y-3">
                                                <Textarea
                                                    ...
                                                    onChange={(e) => setRejectionReason({ ...rejectionReason, [ex.id]: e.target.value })}
                                                />
                                                <Button
                                                    variant="ghost"
