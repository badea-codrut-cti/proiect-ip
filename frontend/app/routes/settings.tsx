...
export default function SettingsPage() {
  ...
  async function handleFsrsRecalculate() {
    ...
    setFsrsLoading(true);
    try {
      const res = await apiFetch<{ message: string }>("/api/auth/fsrs/recalculate", {
        method: "POST",
      });
      setMessage(res.message || "FSRS parameters recalculated successfully");
    } catch (err) {
      setError(err instanceof Error ? err.message : "Failed to recalculate FSRS parameters");
    } finally {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900">
      ...
      <main className="max-w-3xl mx-auto p-6">
        ...
        <div className="mt-6 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100 mb-4">Learning Algorithm</h2>
          <div className="space-y-4">
            <p className="text-sm text-slate-600 dark:text-slate-300">Reset FSRS (Free Spaced Repetition System) parameters to restart your learning schedule.</p>
            <button 
              type="button" 
            ...
            >
              {fsrsLoading ? 'Resetting...' : 'Recalculate FSRS Parameters'}
            </button>
          </div>
        </div>
      </main>
