...
import { contributionsClient } from "~/utils/contributionsClient";
...
function mainDateFor(tab: Tab, it: any): Date | null {
  return tab === "exercises" ? safeDate(it.created_at) : safeDate(it.edited_at);
}
...
function AdminFeedback({ text }: { text: string }) {
  return (
    <div className="mt-4 rounded-2xl border border-rose-200/70 dark:border-rose-900/40 bg-rose-50/70 dark:bg-rose-900/15 p-4">
      ...
      <div className="mt-2 text-sm leading-relaxed text-slate-700 dark:text-slate-200 whitespace-pre-wrap">{text}</div>
    </div>
  );
}
...
export default function ContributionsPage() {
  ...
  const items = useMemo(() => {
    ...
    filtered.sort((a: any, b: any) => {
      ...
      const ad = mainDateFor(tab, a)?.getTime() ?? 0;
      const bd = mainDateFor(tab, b)?.getTime() ?? 0;
      return (ad - bd) * dir;
    });

    return filtered;
  }, [rawItems, tab, statusFilter, query, sortKey, sortDir]);
      ...
      <main className="max-w-6xl mx-auto px-4 py-10">
          ...
          <div className="flex flex-col lg:flex-row gap-3 lg:items-center lg:justify-between">
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
              ...
              <select
                ...
                className="h-10 rounded-xl px-3 text-sm bg-white dark:bg-slate-900 ring-1 ring-slate-200 dark:ring-slate-800 outline-none"
              >
                <option value="created">{tab === "exercises" ? "Submitted date" : "Edited date"}</option>
        ...
        {loading ? (
          ...
          <div className="grid gap-6">
            {items.map((it: any) => {
              const created = mainDateFor(tab, it);
                      ...
                      <div className="min-w-0">
                        ...
                        <CardDescription className="mt-2 text-sm">
                          <span className="text-slate-500 dark:text-slate-400">
                            {tab === "exercises" ? "Submitted:" : "Edited:"}
                          </span>{" "}
                  ...
                  <CardContent className="pt-0 pb-6">
                    {tab === "exercises" ? (
                      <div className="space-y-3">
                        ...
                        {it.status === "rejected" && it.rejection_reason && <AdminFeedback text={it.rejection_reason} />}
                      </div>
                    ) : (
