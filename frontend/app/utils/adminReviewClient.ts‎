...
const endpoints = {
  ...
  approveExercise: (id: string) =>
    `/api/exercises/${encodeURIComponent(id)}/approve`,
  rejectExercise: (id: string) =>
    `/api/exercises/${encodeURIComponent(id)}/reject`,

  // Approve / Reject COUNTER-EDITS
  approveCounterEdit: (id: string) =>
    `/api/counter-edits/${encodeURIComponent(id)}/approve`,
  rejectCounterEdit: (id: string) =>
    `/api/counter-edits/${encodeURIComponent(id)}/reject`,
};
...
export type PendingSentenceEdit = {
  ...
  // CURRENT (from counters.documentation)
...
export const adminReviewClient = {
  getCounts: async (): Promise<AdminCounts> => {
    const [appsRaw, exercisesRaw, editsRaw] = await Promise.all([
      apiFetch<
        PendingContributorApplication[] | { items: PendingContributorApplication[] } | unknown
      >(endpoints.pendingContributorApps),
      apiFetch<PendingSentenceNew[] | { items: PendingSentenceNew[] } | unknown>(
        endpoints.pendingExercises
      ),
      apiFetch<PendingSentenceEdit[] | { items: PendingSentenceEdit[] } | unknown>(
        endpoints.pendingCounterEdits
      ),
    ]);
  ...
  getPendingNewSentences: async (): Promise<{ items: PendingSentenceNew[] }> => {
    const raw = await apiFetch<
      PendingSentenceNew[] | { items: PendingSentenceNew[] } | unknown
    >(endpoints.pendingExercises);
  ...
  getPendingEditedSentences: async (): Promise<{ items: PendingSentenceEdit[] }> => {
    const raw = await apiFetch<
      PendingSentenceEdit[] | { items: PendingSentenceEdit[] } | unknown
    >(endpoints.pendingCounterEdits);
