...
router.get("/pending", sessionMiddleware, adminMiddleware, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ error: "Failed to fetch pending counter edits" });
  }
});
  ...
  try {
    ...
    const result = await authService.getPool().query(
      `INSERT INTO counter_edits 
       ...
       VALUES 
         (gen_random_uuid(), $1, $2, $3, 'pending', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) 
       RETURNING 
         id, created_by, approved_by, counter_id, content, status,
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      await authService.getPool().query(
        ...
        [
          edit.created_by,
          `Your documentation edit for ${edit.counter_name} has been rejected. Reason: ${reason || "No reason provided"}`,
